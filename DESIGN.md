...
| Tauri app (in-app keys) | `kbd-core` + `kbd-tao` | Tauri uses `tao` (winit fork) with its own `KeyCode` |
...
| GPUI (Zed) app | `kbd-core` + `kbd-gpui` | Bridge for GPUI's string-based `Keystroke` type |
| Dioxus app | `kbd-core` | `Key::from(code)`, no bridge crate needed |
| Floem app | `kbd-core` | Uses `keyboard_types::Code` via `ui_events`, no bridge needed |
...
| `kbd-gpui` | GPUI (Zed) | String-based `Keystroke` + boolean `Modifiers` |
...
Most GUI frameworks derive their key types from the same W3C spec, so
conversions are mechanical 1:1 mappings. GPUI is the exception — it
uses string-based key names, not an enum. crossterm and termion use
logical key models (characters, not physical positions). Some
frameworks don't need a bridge crate at all: Dioxus and floem both
use `keyboard_types::Code` directly (floem via the `ui_events` crate).

Not every crate needs to exist at launch. The TUI and GUI bridges
(`kbd-crossterm`, `kbd-winit`, `kbd-tao`, `kbd-iced`, `kbd-egui`,
`kbd-gpui`) are built first. The rest (`kbd-termion`, `kbd-makepad`,
`kbd-gtk`) are niche and built on demand.
...
`ui_events`, but iced, egui, tao, GPUI, and Makepad each define
their own key types. Only Dioxus and floem can use `kbd-core`
directly without a bridge crate.
...
| tao (Tauri) | Own `KeyCode` (W3C-derived, winit fork) | `kbd-tao` (1:1 mapping) |
| iced | Own `Code` (W3C-derived) | `kbd-iced` (1:1 mapping) |
| egui | Own `Key` enum (smaller, not 1:1) | `kbd-egui` |
| GPUI (Zed) | Own `Keystroke` (string key names) + `Modifiers` (booleans) | `kbd-gpui` (string-to-enum mapping) |
| Makepad | Custom `KeyCode` | `kbd-makepad` |
| gtk-rs | GTK native events (GDK keysyms) | `kbd-gtk` |
| crossterm | `KeyCode::Char('a')` (logical) | `kbd-crossterm` |
| termion | `Key::Char('a')` (logical, mods baked in) | `kbd-termion` |
| Tauri (global hotkeys) | N/A — uses `kbd-global` as Linux backend | No bridge crate, uses evdev+portal |
