...
Global hotkey runtime for Linux — threaded engine, device management, and
backend selection. Works on Wayland, X11, and TTY.
...
All `kbd` domain types (`Key`, `Modifier`, `Hotkey`, `Matcher`, `Layer`, etc.)
are re-exported so you only need a single dependency.

## Permissions

Your user must be able to read `/dev/input/event*` devices. On most systems
this means joining the `input` group:
...
Grab mode uses `EVIOCGRAB` for exclusive device capture and writes to
`/dev/uinput` to re-emit non-hotkey events. Grant access with a udev rule:
