...
// evdev conversions — gated behind the `evdev` feature flag.
//
// These must live in kbd-core (the crate defining `Key`) due to the orphan
// rule: `impl From<ForeignType> for LocalType` requires the impl to be in
// the crate that defines the local type.
#[cfg(feature = "evdev")]
mod evdev_conversions {
    use evdev::KeyCode;
