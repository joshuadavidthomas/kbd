...
### 4.3 Modifier aliases — DEFERRED

**Status:** Deferred. Implementation complete in draft PR
[#122](https://github.com/joshuadavidthomas/kbd/pull/122) but not
merged. The feature adds significant internal complexity (non-`Copy`
`Modifier`, shadow lookup table, alias resolution threaded through
layers/sequences/queries/introspection) for something consumers can
handle themselves at the config layer — resolve the alias to a
concrete modifier before registering bindings. The only capability
this provides beyond that is runtime alias reassignment without
re-registration, which is a niche use case with no demonstrated
consumer need. Will revisit if a downstream project requires it.
...
- [x] `BindingSource` newtype (wraps a string label: `"default"`, `"user"`, `"plugin"`, or custom).
- [x] `BindingOptions::source(BindingSource::new("user"))`.
- [x] Introspection API (§3.5) returns source info per binding.
- [x] Optional source-aware precedence: user-sourced bindings override default-sourced for the same hotkey, without requiring explicit unregister + re-register.
- [x] Tests: source tagging, source in introspection results, source-aware conflict resolution.

### 4.5 Device-specific bindings

- [x] `BindingOptions::device(DeviceFilter::Name("..."))` restricts binding to specific devices.
- [x] Device filter matching in the engine's matcher (per-binding check).
- [x] Per-device modifier isolation (modifier on device A doesn't satisfy binding on device B).
- [x] Global bindings use aggregate modifier state.
- [x] Tests: device match, device miss, modifier isolation, global aggregate.

Reference: `archive/v0/src/listener/dispatch.rs` (device-specific dispatch)

### 4.6 Debounce, rate limiting, and repeat policy

- [x] Per-binding debounce (suppress triggers within time window).
- [x] Per-binding rate limit (cap invocations per interval).
- [x] `BindingOptions::repeat_policy(RepeatPolicy)` — `Allow`, `Suppress`, `Custom { rate, delay }`.
- [x] Engine filters evdev repeat events per-binding based on policy. Distinct from debounce: repeat is the OS auto-repeating a held key, debounce suppresses rapid re-presses.
- [x] Tests: debounce suppression, rate limiting, repeat suppression, custom repeat rate, interaction between debounce and repeat.
...
- [x] Tap resolves on release before threshold.
- [x] Hold resolves on threshold expiry.
- [x] Hold resolves on interrupting keypress (keyd model).
- [x] Tap-hold requires grab mode; clear error without it.
- [x] Tests: tap, hold by duration, hold by interrupt, missing-grab error.
...
| 4.1 Sequences | 10/10 |
| 4.2 Serde | 4/4 |
| 4.3 Modifier aliases | DEFERRED ([#122](https://github.com/joshuadavidthomas/kbd/pull/122)) |
| 4.4 Provenance | 5/5 |
| 4.5 Device filtering | 5/5 |
| 4.6 Debounce/rate/repeat | 5/5 |
| 4.7 Tap-hold | 5/5 |
